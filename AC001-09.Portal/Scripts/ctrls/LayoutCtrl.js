app.controller('LayoutCtrl', ['$scope', '$element', '$window', '$http', '$interval', '$location', 'signalrService', '$timeout',
    function ($scope, $element, $window, $http, $interval, $location, signalrService, $timeout) {
        ...
        $scope.lineChatClick = function () {
            var lineChatDate = new Date().getTime();
            var lineChatInfo = $scope.ContactInfo.Live800Link;
            if (lineChatInfo.indexOf('?') != -1) {
                lineChatInfo = lineChatInfo + '&_=' + lineChatDate;
            } else {
                lineChatInfo = lineChatInfo + '?_=' + lineChatDate;
            }
            var linechat = $window.open(lineChatInfo, '_open', 'width=588,height=486');
            linechat.focus();
        };
